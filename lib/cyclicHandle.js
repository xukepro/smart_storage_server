...
class CyclicHandler {
  ...
  cyclicLoad () {
    ...
    return _this.redisClient.getNewRequests(
      _this.redisCfg.key,
      _this.redisCfg.loadTimeInterval)
      .then(function (response) {
        ...
        Promise.all(actions).then(function (res) {
          ...
          if (!_this.enbmapCfg) return;

          // return results to map
          for (let uid in _this.maps) {
            var map = _this.maps[uid];
            if (!Object.prototype.hasOwnProperty.call(results.tags, map.selectedTag)) return;
            _this.log.info('return ' + map.selectedTag + ' message to map: ' + uid);
            var tag = results.tags[map.selectedTag];
            if (uid.indexOf('anyplace') != -1 || typeof (map.floorInfo) != 'undefined') {
