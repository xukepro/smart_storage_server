...
function fusion(kalmanFilter, input, result) {
  if (!('previous' in input) || input.previous.length === 0) {
    return 0;
  }
  let obs = input.previous[input.previous.length - 1];
  ...
  if (!kalmanFilter.Inited) {
    kalmanFilter.init(
      X0 = obs.pos,
      P0 = [[1, 0], [0, 1]],
      R0 = [[Math.pow(2.2, 2), 0], [0, Math.pow(2.2, 2)]],
      Q0 = [[Math.pow(0.6, 2), 0], [0, Math.pow(0.6, 2)]]
    );
  }
...
function CUPT(kalmanFilter, refpoint, result) {
  if ((!kalmanFilter.Inited) || !(refpoint[0] in rcoords)) {
    return 0;
  }
